...
1. **Check MCP Availability**: Verify Assessment MCP server is available
   - If MCP server is not configured, return immediately with instructions
...
Ensure the Assessment MCP server is configured. If not available, the skill will return instructions for setup.
...
**MCP Not Available**:
• Return immediately with setup instructions
